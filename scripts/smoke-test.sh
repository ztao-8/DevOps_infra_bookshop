...
# 添加一本书
curl -s -o /dev/null -w "%{http_code}" -X POST -H "Content-Type: application/json" -d '{"title": "Test Book3", "description": "Test Desc", "price": 10.99, "cover": "http://test.com/cover.jpg"}' "$API_URL/books" | grep 201> /dev/null && echo "✅ POST /books passed" || echo "❌ POST /books failed"
